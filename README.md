...
- In the editor, it presents a **visual editor by default** — structured controls for building and editing the song, alongside a **live sheet-music preview** that mirrors the front-end render — and keeps **raw-JSON editing available** as an alternative behind a mode switch. Both modes edit the **same single song** in the plugin's own [song format](docs/song-format.md). The raw-JSON field's input is **validated for conformance**, but that check is **informational only — it never blocks saving**, and your raw text is always stored.
...
| `src/edit.js` | The block's editor component (JSX) — a **mode container** that shows the **visual editor + live preview** by default and switches to a **raw-JSON** field (`TextareaControl` + non-blocking error `Notice`) in JSON mode. The mode toggle is editor-only UI state; both surfaces edit the same `song` string. |
| `src/editor/` | The **visual editor UI**: the structured drill-down editors (song / section / measure / event / pitch) and the empty / invalid states, the shared list controls and breadcrumb, the per-song note-name and song-shape helpers (working object, serialize-time conformance guard, accessible name), and the live, read-only sheet-music **preview** that reuses the notation core in `src/notation/`. Built only on `@wordpress/*` packages (no new runtime dependency). |
| `src/editor/__tests__/` | Jest unit tests for the editor (the mode container, the structured editors, the preview, and the helpers), run by `npm run test:unit`. |
