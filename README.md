...
### The now-dormant `interactive` hit-rect

The editor and the front end share the **same** notation emit (`src/notation/svg.js`). That emit accepts an opt-in **`interactive`** flag: when set, it draws one invisible, pointer-hittable `<rect>` per event behind the visible glyphs, giving each note and rest a solid click target (hit-testing a note's painted `<g>` directly is unreliable — the group's drawn children leave gaps a click can fall through). The flag exists because canvas click-to-select once needed it. With **click-to-select removed**, neither surface passes it anymore: the canvas (`SongCanvas`) renders **without** `interactive`, exactly like the front end (`src/view.js`), so the **editor SVG is now byte-identical to the front end's**. The flag and the hit-rect code **remain in `svg.js`** (defaulting off) but are **dormant/unused** — `svg.js` was not changed and the flag was not deleted, so the capability stays available behind the default-false flag should a future surface need it. The front-end render contract is unchanged.
