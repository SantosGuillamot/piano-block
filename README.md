...
The canvas **re-renders live** as you edit and draws the song with the **same notation the published page uses by default** — it *is* the live render, highlighting the current selection, not a separate read-only preview. The match is exact while the published page's note names are off (the default): the staff geometry is the same on both surfaces. The published page can additionally **overlay the note names** (a viewer control the editor never has) on top of that identical geometry, so the only thing the front end can show that the canvas never does is those optional names.
...
Song *storage*, **visual authoring**, front-end **notation rendering**, and the front end's **first viewer control** have landed, but the audible and richer interactive piano experience has not. Authoring now happens through a **structure tree** beside the canvas — you navigate and select the song there and adjust the selected node's settings in the **block settings sidebar**, while the **canvas is the live render with selection highlighting** (raw-JSON editing stays available as an alternative — see [Using the Piano block](#using-the-piano-block)). The published page no longer only renders the score: it now also carries a **viewer-operable control** — the note-name toggle — that shows or hides the note names over that score (off by default, so the default render is unchanged; see [Using the Piano block](#using-the-piano-block)). The rendered notation already covers a broad spread of markings, including **gradual dynamics** — crescendo and decrescendo hairpins drawn as `<` / `>` wedges (see the [song format reference](docs/song-format.md) for the fields and their v1 limits). The following are planned for future tasks and are **not** part of v1 (the "out of scope" set):
