...
The Piano block stores one **song** — a JSON document in the plugin's own [song format](docs/song-format.md). You author that song with a **visual editor** (the default surface): you **navigate and select** the song through a **structure tree** — a toggleable outline panel beside the canvas (Section → Measure → Right/Left hand → Note) — run add/remove/duplicate at every level from that tree, and configure the selected node in the **block settings sidebar**, while the **canvas is the live render with selection highlighting** (display + highlight only). If you prefer, you can switch to editing the **raw JSON** of the same song as text. Here is the end-to-end workflow.
...
**Browse and select with the structure tree.** A **toolbar button (Structure)** toggles a **structure tree** open and closed beside the canvas (to its left). The tree lists the whole song hierarchically — **Section → Measure → Right hand / Left hand → Note** — with sections and measures **expandable and collapsible**, so you can drill from the song down to an individual note. **Selecting** a section, measure, or note row in the tree opens its settings in the **block settings sidebar** and **highlights and scrolls to** the matching element on the canvas; the canvas reflects the selection but **clicking the staff does not select**. The **Right hand / Left hand** rows are **organizational** — they expand a measure's two event lists and host that hand's **Add note** button — so they have no settings panel of their own, and selecting one does not change the sidebar.
...
**Add, remove, and duplicate from the tree.** At **every level** the tree gives you **add, remove, and duplicate** controls — for sections, measures, and notes (a note is added under a measure's **Right hand** or **Left hand** group). **Add section** lives at the bottom of the tree, **Add measure** on each section row, and per-hand **Add note** on each hand-group row — that per-hand "Add note" is how you seed the first note of an otherwise empty measure. **Duplicate** makes a **deep copy inserted immediately after the original**: duplicating a section copies its measures, notes, and its name; a measure copies both hands and its name; a note copies its pitches and properties. **Reordering is not available** — the tree has no move controls.
...
- A **Section** panel for a selected section — a **Name** field (see below) and, behind *Advanced*, the section's overrides. A convenience **Add section** / **Remove section** also appears here.

A **Song** panel is **always present** in the sidebar, whether or not anything is selected: it holds the song's **title and composer** and the song-wide musical **defaults** — tempo, time signature, and (behind *Advanced*) the beat unit and each hand's clef, accidentals, and octave shift. When nothing is selected, the sidebar shows just this Song panel.
...
| `src/edit.js` | The block's editor component (JSX) — a **mode container** that shows the **interactive sheet-music canvas + a block settings sidebar (`InspectorControls`) of panels** by default and switches to a **raw-JSON** field (`TextareaControl` + non-blocking error `Notice`) in JSON mode. It also owns the **structural mutators** (add/remove section and measure) lifted out of the canvas, so the canvas and the sidebar share one mutation path. The mode toggle and the current **kind-tagged selection** (section / measure / event) are editor-only UI state; both surfaces edit the same `song` string. |
| `src/editor/` | The **visual editor UI**: the **interactive sheet-music canvas** (`SongCanvas`, which reuses the notation core in `src/notation/` to draw the working song and to hit-test note selection via the `data-*` hooks and per-event hit-rects the core emits — it is now **selection + decoration only**, drawing the active highlight; the on-canvas add-note / add-measure affordances were removed), the **block settings sidebar panels** (`inspector/` — Song and **Structure** panels always present, plus Note / Measure / Section panels for the current selection; the `StructureList` is the browsable sections → measures list with add/remove/select), the pure **selection helpers** (`selection.js`, mapping a click to a `kind`-tagged `(section, measure, hand, event)` selection), the invalid-state fallback, and the reused leaf field-editors and per-song note-name / song-shape helpers (working object, serialize-time conformance guard, accessible name). Built only on `@wordpress/*` packages (no new runtime dependency). |
...
### The editor-only `interactive` hit-rect

The editor and the front end share the **same** notation emit (`src/notation/svg.js`), but the editor has one extra need: a reliable pointer target for selecting a note. Hit-testing a note's painted `<g>` directly is unreliable — the group's drawn children (notehead, stem, flag) leave gaps a click can fall through. So the emit accepts an opt-in **`interactive`** flag: when set, it draws one invisible, pointer-hittable `<rect>` per event behind the visible glyphs, giving each note and rest a solid click area. The canvas (`SongCanvas`) passes `interactive: true`; the front end (`src/view.js`) passes **no** flag, so the published SVG omits the hit-rects entirely and is **byte-identical** to before. Selection therefore stays a purely editor-side concern, and the front-end render contract is unchanged.
