...
2. **`chordSymbol` removal (clean break).** Every `chordSymbol` token — identifiers *and* prose comments/JSDoc — is removed across the 13 in-scope files in `src/`, `specs/`, `docs/`, and `README.md`. The size constant `CHORD_SYMBOL_SIZE` is renamed `NOTE_SIZE`; the emit text-kind value `chord-symbol` becomes `note`. The two behaviors that must survive — above-RH free-text rendering and XSS-inert verbatim text — transfer 1:1 with **zero new safety code**, because both mechanisms are field-agnostic. Legacy `chordSymbol` data stays valid (the schema is permissive about unknown keys) and simply does not render.
...
`additionalProperties` is permissive everywhere, so:
- A stray `staff` or `beat` on a per-event note is silently ignored (no error) — satisfies the "stray field ignored" criterion with no extra code.
- A legacy `chordSymbol` after removal is an unknown optional key → silently valid → does not render.
...
- **Choice:** Resolve four band Y anchors per system. above-RH reuses the `topMarginLayout` lane (renamed). The inter-staff gap holds two sub-bands — below-RH grows down from `rightStaffBottomY`, above-LH grows up from `leftStaffTopY`. below-LH grows down from `leftStaffBottomY` into the bottom margin. The inter-staff gap **always flexes**: `effectiveInterStaffGap = max(INTRA_STAFF_GAP, belowRH_stack + aboveLH_stack + (both_present ? MID_GAP : 0))`, collapsing to the base 8 sp when empty. The bottom margin flexes the same way for below-LH. below-RH/below-LH notes dodge below their hand's dynamics row only when that hand has dynamics in the system. The flex is computed **before** `lhTopY` is set (the one pipeline reorder: occupancy → gap → `lhTopY`). No general cross-kind (note-vs-dynamic/ottava) collision solver is built.
...
- **New constants (`src/notation/constants.js`):**
  ...
  - `DYNAMICS_LANE_RESERVE` (~`4.5`) — the dynamics glyph box height a below-RH/below-LH note dodges past when the hand has dynamics; a tunable constant, not a hard derivation.
...
- **Stray `staff`/`beat` on a per-event note, or a legacy `chordSymbol`** → permissive `additionalProperties` ignores them; no error, no render.
...
- **Constant tuning (low risk, plan/code phase).** The exact values of `NOTE_GAP_STAFF`, `MID_GAP`, `DYNAMICS_LANE_RESERVE`, and `NOTE_CLAMP_INSET` are given as ranges grounded in text-metric arithmetic (`NOTE_SIZE = 2.8`, ascent ≈ 2.02 sp, descent ≈ 0.62 sp, step = 3.4 sp). The 1+1 inter-staff case computes to ≈ 8.46 sp (exceeds the base 8 by ≈ 0.5), confirming the flex is needed. Final values should be picked in code and confirmed visually; because every band/height test is relative or existence-only, tuning will not break tests.
...
- **`chordDyR`/`chordDyL` generalization.** The old chord-lane local-frame conversion in `svg.js` (`band.chordSymbolY − staffBottomY`) must be generalized to read the four band anchors. The plan should confirm whether the note branch reads band anchors directly or via a small per-hand conversion helper analogous to the old `chordDy`.
