...
The work is sequenced so each layer lands with its tests before the next depends on it: (1) schema data-model swap, (2) constants rename + new constants, (3) the `chordSymbol` token removal / `note`-vocabulary rename across code + tests + docs, (4) per-event `notes` collection in layout, (5) standalone `notes` collection + beat→X interpolation in layout, (6) the four-band geometry with the PAIR A flex + dynamics dodge, (7) per-event emit with band Y + observability attributes, (8) standalone emit with raw band Y + `data-staff`, (9) end-to-end coverage. The validator (`src/song/validate.js`) is intentionally **not changed** — every new rule is expressible declaratively in the schema.
...
### Task 2: Constants — rename `CHORD_SYMBOL_SIZE → NOTE_SIZE`; add the four new note constants

- **Goal:** Provide the renamed size constant and the new geometry constants the layout/emit work consumes, before any consumer references them.
- **Files:**
  - `src/notation/constants.js`
- **Changes:**
  - Rename the export `CHORD_SYMBOL_SIZE` (currently line ~158, value `2.8`) to `NOTE_SIZE`, keeping value `2.8`, and reword its JSDoc from "Chord-symbol text size (free author text above the RH staff)" to describe note-annotation text size (free author text). Do **not** yet touch the importers — Tasks 3/6/7 update `layout.js` and `svg.js` imports.
...
- **Acceptance:**
  - `NOTE_SIZE` is exported with value `2.8`; `CHORD_SYMBOL_SIZE` is no longer exported (a unit test importing `NOTE_SIZE` resolves; nothing in `constants.js` references `CHORD_SYMBOL_SIZE`).
  ...
  - (At this point `layout.js`/`svg.js` still import `CHORD_SYMBOL_SIZE` and will not build/test until Tasks 3/6/7 update them; this task is complete when the constants module itself is correct and its values are importable. The code-writer should run only the constants-scoped check here and not expect the full suite green until Task 3.)
...
- **Goal:** Eliminate the `chordSymbol` token everywhere it remains, fix the `layout.js`/`svg.js` imports broken by Task 2's rename, and replace the chord-lane vocabulary with the `note` vocabulary so the tree builds again — while preserving the two locked behaviors (above-RH free-text rendering, XSS-inert verbatim text) by retargeting (not deleting) their tests onto `notes`. This is the bridge that keeps the tree green between the schema/constants changes and the new band model.
- **Files:**
...
- **Changes:** (this is a transitional rename; the full four-band model arrives in Task 6. The above-RH single-lane behavior is preserved 1:1 here so nothing regresses mid-sequence.)
  - **`src/notation/layout.js`:**
    - Update the import from `./constants.js`: replace `CHORD_SYMBOL_SIZE` with `NOTE_SIZE`.
  ...
  - **`src/notation/svg.js`:**
    - Update the import: replace `CHORD_SYMBOL_SIZE` with `NOTE_SIZE`.
...
- **Depends on:** Task 1 (schema), Task 2 (constants rename + `NOTE_SIZE`)
...
- **Acceptance:**
  - `grep -rn "chordSymbol\|chord-symbol\|CHORD_SYMBOL"` across `src/`, `specs/`, `docs/`, `README.md` returns no matches.
  - The full unit suite passes (`npm run test:unit`): the retargeted layout/svg tests pass against the `note` vocabulary.
...
- **Goal:** Resolve `measure.notes` into a measure-level `standaloneNotes` collection with each note's horizontal X computed by interpolating its `beat` onset over the justified column grid (with the over-content clamp), carrying `text`, `placement`, `staff`.
- **Files:**
  - `src/notation/layout.js`
  - `src/notation/__tests__/layout.test.js`
- **Changes:**
  - Add a new pure helper `collectStandaloneNotes(measureNotes, ctx)` (or inline at the measure-walk site) that runs at the measure-walk inside `buildLayoutModel` (currently ~1753–1855), where `columnX`, `ml.columns`, `ml.measureEnd`, `leadInset`, `measureRightX`, and `advanceScale` are all in scope. For each element of `measure.notes` with a non-empty string `text`, produce `{ kind: "note", x, text, placement, staff }`.
  - **X resolution** (per the X-anchoring decision):
    - A no-`beat` note is treated as `beat: 0`, which maps to the measure's content-left edge (`leadInset`).
    - For a `beat` value, interpolate over the *scaled* `columnX` grid: find the bracketing onsets `[t_i, t_next]` in the sorted onset grid where `t_i ≤ beat < t_next`; `x(beat) = lerp(columnX(t_i), nextX, (beat − t_i) / (t_next − t_i))`. The last column extends to `measureEnd`/content right edge; guard the span denominator with `|| 1` to stay NaN-safe. (Use the same scaled column X positions the per-event notes use, so a standalone `beat` that coincides with an event column lands at that event's X.)
    - **Over-content clamp:** `x = min( beatToX(min(beat, measureEnd)), measureRightX − NOTE_CLAMP_INSET )` so an over-content `beat` (e.g. 99) renders strictly inside the measure content, never straddling the trailing barline. (`measureRightX` here is measure-local — the same value the barline uses, expressed relative to the measure group; ensure the value stored on the standalone primitive is in the same coordinate frame the per-event/measure-relative X values use so the emit can place it under the `<g data-measure>` translate.)
  ...
  - Do not compute Y here — band Y is Task 6/7. The standalone primitive carries `x`, `text`, `placement`, `staff` only.
  - Grouping for stacking is by the **raw** `beat` (`beat ?? "noBeat"`) per `(staff, placement)` — record enough on the primitive (or keep array order within a group) that Task 6/7 can stack same-anchor notes; per the stacking decision, two `beat: 2` notes are one group (k = 0, 1) while `beat: 2` and `beat: 2.0001` are distinct groups at distinct interpolated X.
- **Depends on:** Task 4
- **Traces to:** Spec requirement 17; design Decision "Per-event X from the event column; standalone X by interpolated beat onset with an over-content clamp" + the `standaloneNotes` naming note. Spec ACs: `beat: 2` further right than `beat: 0`; no-`beat` ≈ `beat: 0` and left of `beat: 2`; over-content `beat: 99` still renders within the system's horizontal bounds.
- **Acceptance:**
  ...
  - For a standalone note with `beat: 99` in a measure whose content ends well before that, the resolved X is ≤ `measureRightX − NOTE_CLAMP_INSET` and within the measure's content extent (does not exceed the trailing barline X).
  - Each `standaloneNotes` record carries `kind: "note"`, `text`, `placement`, and `staff`.
...
- **Files:**
  ...
  - `src/notation/__tests__/svg.test.js`
- **Changes:**
  - In `renderMeasure` (~469–506): after rendering both hands' primitives, iterate `measure.standaloneNotes` (from Task 5). For each, select the band by `(staff, placement)` and compute note #k's **raw band anchor Y in system coordinates** (per Task 6's band anchors) — these notes live under `<g data-measure transform="translate(measure.x 0)">`, which has **no Y translate**, so the Y is NOT run through the `bandY − staffBottomY` conversion (applying the per-event conversion here would misplace the node). Stack k by the note's index within its `(staff, placement, raw beat ?? "noBeat")` group from Task 5.
  - Emit each as `<text x={standalone.x} y={rawBandY} fill=INK font-size=NOTE_SIZE text-anchor="middle" data-text="note" data-staff={standalone.staff} data-placement={standalone.placement}>` written via `setText`. The node is a direct child of the `<g data-measure>` group (not inside any `<g data-hand>`).
...
- **Acceptance:**
  ...
  - Two standalone notes with `beat:0` and `beat:2` emit two nodes, the `beat:2` node further right than the `beat:0` node.
  - A standalone note with `beat:99` emits a `<text>` node within the system's horizontal bounds (X ≤ `measureRightX − NOTE_CLAMP_INSET`).
  - A measure containing both a per-event note and a standalone note emits both nodes.
  - N standalone notes of the same `(staff, placement, beat)` emit N nodes at N distinct Ys.
  - `npm run test:unit` passes; `npm run check` passes.
