...
9. **Rework `Edit.test.js`'s tree-drill helpers for the select-only label (REQUIRED — T4's commit is red without it).** `Edit.test.js` today drives the tree by **clicking label rows to expand them**: its `treeRowButton`/`clickByText` helpers (lines 109–118) find a **`button`** and caret-strip its text, and `selectLoneNote` (lines 386–391) plus the "seeds a note from the structure tree's per-hand Add note" test (lines 305–334) do `clickByText(container, "Section 1")` → `clickByText(container, "Measure 1")` → `clickByText(container, "Right hand")` (→ `clickByText(container, "C")`), each expecting the prior click to **expand** so the next child row renders. Today that works because the label `onClick` calls `toggleRow(...)` then `onSelect(...)` (expands-and-selects). **After T4 the label is select-only** (toggle moved to the non-focusable chevron), so `clickByText(container, "Section 1")` selects without expanding, "Measure 1" never renders, and the next lookup returns `undefined` and `.click()` throws. The chevron is a non-focusable `<span>`/`<Icon>` sibling (the components mock's `Icon` renders `<span data-icon ...rest>`, and it spreads `...rest` so the chevron's pointer `onClick` is present in the DOM and clickable in jest) — it is **not** a `button`, so `treeRowButton`/`clickByText` cannot reach it. Required rework:
   ...
   - **General rule (apply to EVERY breaking site, not a fixed line-list).** Any `Edit.test.js` site that reaches a **child** row by clicking an **ancestor's label** must move to the expand helper (chevron click or a seeded `expanded` Set) for the ancestor click(s), because the select-only label no longer expands. A click that only **selects** an already-visible row (a single label click, no descendant needed afterward) stays a `clickByText`/`treeRowButton` select and does NOT change. Concretely, the breaking sites (each does `clickByText("Section 1")` expecting expansion, then a child click) are:
     - **`selectLoneNote` (lines 386–391)** — `Section 1` → `Measure 1` → `Right hand` → `C`. Rewrite to expand Section 1, Measure 1, Right hand via the expand helper (or seed `expanded`), then `clickByText("C")` to select. Reworking this one helper fixes **all seven of its callers** (lines 342, 360, 396, 419, 436, 466, 503).
     - **The "seeds a note from the structure tree's per-hand Add note" test (lines 305–334)** — inline `Section 1` → `Measure 1` → `Right hand` chain (lines 312–314). Expand those three via the helper so the hand-group row's `Add note to Right hand of measure 1 of section 1` button is reachable, then click it as today.
     - **`describe("Edit — structure tree")` inline chains** that do not route through `selectLoneNote`:
       - **"selecting a tree measure row reveals the Measure and Section panels" (line 552)** — `clickByText("Section 1")` (lines 555) then `clickByText("Measure 1")` (556). Rewrite to `expandRow(... "Section 1")` then `clickByText("Measure 1")` to **select** the measure.
       - **"selecting a tree note row reveals all three per-level panels" (line 562)** — `Section 1` → `Measure 1` → `Right hand` → `C` (lines 566–570), all label-click-expands. Rewrite the first three to the expand helper, then `clickByText("C")` to select.
     - **Safe sibling (do NOT change): "selecting a tree section row reveals only the Section panel" (line 541)** — a single `clickByText("Section 1")` (line 544) that only selects a top-level, already-visible row; no descendant is clicked afterward, so it stays as-is.
   - For the hand-group level: the hand-group **label** still toggles on click per T4 (it is the non-selecting expander), so either `expandRow` (chevron) OR a hand-label click expands — use the same mechanism the helper uses everywhere for consistency.
   - The downstream assertions in all these tests (panel gating, persisted-song shape, `validateSong` empty) are **unchanged** — only the drill-down mechanics change.
...
- `Edit.test.js` passes **with every ancestor-label-expands drill-down site reworked for select-only labels** (change #9, the general rule). The `treeRowButton`/`clickByText` helpers (lines 109–118) drop the caret-strip regex; `clickByText` is select-only after the rework. The sites that must change are exactly the ones that reach a child row by clicking an ancestor label:
  - **`selectLoneNote` (lines 386–391)** becomes expand-then-select — this single helper rework covers **all seven of its callers** (lines 342, 360, 396, 419, 436, 466, 503), i.e. `"inserts a contextual note right after the selected event…"`, `"clears a stale selection after a raw edit…"`, `"shows all three per-level panels for an event-kind tree selection"`, and the four other callers.
  - **`"seeds a note from the structure tree's per-hand Add note and opens the Note panel"` (line 305)** — its inline `Section 1` → `Measure 1` → `Right hand` chain (lines 312–314) becomes expand-via-helper, then the per-hand `Add note` button click.
  - **`"selecting a tree measure row reveals the Measure and Section panels"` (line 552)** — `Section 1` becomes expand, then `Measure 1` select.
  - **`"selecting a tree note row reveals all three per-level panels"` (line 562)** — `Section 1`/`Measure 1`/`Right hand` become expand, then `C` select.
  - The single-select sites that do **not** change: `"selecting a tree section row reveals only the Section panel"` (line 541, one top-level select click) and all the JSON-mode/empty-seed/invalid-routing/toolbar-toggle tests (none drill into child rows).
