...
This is the one finding with real design content. The **source change** is exactly as the spec specified; the **e2e fallout** is larger than the spec assessed and is resolved here.
...
### 5.3 The e2e fallout (spec under-counted this — resolved here)
...
**Full blast radius — 8 tests break, 1 survives:**
...
| `:580` | (survives) | label-click re-opens `s0` | none (re-confirm in run) |

`:1294` is the site the spec missed entirely: it does **not** use `expandRow` at all, but hard-asserts the collapsed-seed premise at `:1313` (`expect(treeRow("Measure 1")).toHaveCount(0)`), which auto-expanding `s0` breaks directly. `:1361` (hand→notes) is fully fixed by Prong 1 alone because B4 does **not** auto-expand the hand key, so its note-level assertion (`:1381` `treeRow("C")` count-0) is unaffected. `:580` survives because its `:601` label-click re-opens `s0` before any measure-level op (fragile, by accident) — it needs no change but is re-confirmed in the real run.

### 5.4 Resolution — two prongs (keep the two-key seed exactly)

The seed (5.2) is unchanged. The e2e collision is contained to one helper edit plus two one-line inserts — all in `specs/editor.spec.js`.
...
2. **B4's e2e collision is resolved by one idempotent helper + two raw collapse inserts, not by editing each affected test.** Trade-off: `expandRow` is no longer a literal "click the chevron" — it now branches on `aria-expanded`. Accepted because the branch matches the helper's documented intent ("Expand … revealing its children"), fixes 6 sites with zero per-test churn, and `aria-expanded` is a stable, verified signal on the real component. The two keyboard tests genuinely need a collapsed start, so they get explicit raw-toggle collapses (the documented existing affordance) rather than a new `collapseRow` helper that two one-liners don't justify. *(This is the resolution of the spec deviation; see item below.)*
...
Both are **test-impact corrections**. B4's source change (the two-key unconditional seed) ships **exactly as specified**.
...
2. **The spec's B4 e2e impact under-counted.** It named only `:469`; the real blast radius is **8 tests** (`:469`, `:530`, `:676`, `:1294`, `:1331`, `:1361`, `:1504`, `:1535`), including `:1294`, which does not use `expandRow` at all but hard-asserts the collapsed-seed premise. The design enumerates and remedies all of them (Section 5).
...
| Finding | Gate | Notes |
| --- | --- | --- |
| **B1** | **Real e2e run** (Section 2.4) | Running the suite **green**, not editing tests to match. The same run re-confirms the B4 e2e edits (same suite). STOP-and-report-blocker path documented; no silent skip. |
...
| **B4 (e2e)** | The B1 e2e run | Helper rewrite (Prong 1) + the two keyboard-test inserts (Prong 2) are all in `specs/editor.spec.js`, gated by the single e2e run. |
