...
> **Process note.** The Q&A was intended to be answered by a `design-doc-researcher` teammate, but its Topic-1 findings did not reach `design-doc-analyst` (an inter-agent delivery problem, diagnosed by the team lead). The team lead authorized `design-doc-analyst` to proceed in **solo research-and-decide mode** (use own tools — repo, web, small experiments — and record evidence-backed, cited decisions, not blocking on the researcher). The investigation is grounded in: the approved spec, the illustrative prompt, the live scaffold under `src/`, established WordPress block-editor + Western-music-notation domain knowledge, and — for the load-bearing WordPress claims — **official WordPress developer documentation verified via web fetch** (citations inline per topic). **Evidence basis is labelled per topic.**
>
> **Verification status of WordPress-runtime claims.** The worktree has **no `node_modules`** (`npm install` not run) and no running WP, so nothing was executed here. The load-bearing WP facts were instead **verified against `developer.wordpress.org`** (the official handbook / function references), authoritative for API contracts. Three are now doc-cited: Topic 4 (string-attribute-as-JSON-intermediary + no-`source`→comment-delimiter serialization + `default`), Topic 6 (`esc_html` + `get_block_wrapper_attributes` returns a pre-escaped string), Topic 5 (`TextareaControl` / `Notice` existence + props). What official docs cannot prove — the **end-to-end runtime behavior in *this* plugin** (the actual save/reload round-trip of a `song` value; the rendered escaping of a hostile payload; the exact installed `@wordpress/components` version's prop signatures) — remains for the **Code phase** to confirm via `npm run build` + a `wp-env` smoke test against **AC1** (round-trip), **AC3** (empty renders nothing), **AC8** (XSS escaping). `design-doc-reviewer` may also re-verify the cited claims.
...
**Evidence basis.** Spec req 1, 9, 10; AC1, AC2, AC6. Scaffold: `src/block.json` currently declares **no** `attributes`; the block is dynamic (`save: null` — `src/index.js` registers with `edit` only; `src/block.json` has `"render": "file:./render.php"`). **WordPress Block API attribute contract verified against the official Attributes handbook** (`developer.wordpress.org`). **Verification status:** the attribute typing/serialization contract is now **doc-cited** (verbatim quotes below); the *end-to-end runtime round-trip in this plugin* (the value actually persisting into the block-delimiter comment and reappearing on reload) **was not executed in-worktree** (no `node_modules`; no running WP) — flagged for the Code phase to confirm via `npm run build` + `wp-env` (this is exactly AC1).

**Authoritative confirmation (WordPress Attributes handbook, verbatim):**
- *"If you'd like to use an object or an array in an attribute, you can register a `string` attribute type and use JSON as the intermediary."* — directly endorses D4.1 (a `string` attribute carrying the song JSON).
- *"Attributes without a `source` will be automatically saved in the block comment delimiter."* — confirms D4.2 (no `source` → serialized into the delimiter comment, delivered to `render.php`).
- *"A block attribute can contain a default value, which will be used if the `type` and `source` do not match anything within the block content. The value is provided by the `default` field."* — confirms D4.3 (`"default": ""`).
- *"The `type` field MUST be one of the following: `null`, `boolean`, `object`, `array`, `string`, `integer`, `number`."* — confirms `string` is a valid declared type.
...
**Sources:** [Block Attributes handbook](https://developer.wordpress.org/block-editor/reference-guides/block-api/block-attributes/) (string-type-as-JSON-intermediary; no-`source`→comment-delimiter; `default`; allowed `type` values).
...
**Sources:** [TextareaControl](https://developer.wordpress.org/block-editor/reference-guides/components/textarea-control/) (`label`/`help`/`value`/`onChange`/`rows`); [Notice](https://developer.wordpress.org/block-editor/components/notice) (`status` ∈ warning/success/error/info; `onDismiss`); [Component Reference](https://developer.wordpress.org/block-editor/reference-guides/components/).
...
**Evidence basis.** Spec req 11, 12, 13; AC3, AC7, AC8; the live `src/render.php` (current body: `<p <?php echo get_block_wrapper_attributes(); ?>> … esc_html_e( … ) … </p>`); Topic 4's decision that `song` is a **string** attribute; established WordPress output-escaping semantics (`esc_html()`, `get_block_wrapper_attributes()`). **Verification status:** **confirmed against the official function references** (`developer.wordpress.org`): `get_block_wrapper_attributes()` returns a "String of HTML attributes" and its source applies `esc_attr()` per value (`$key . '="' . esc_attr( $value ) . '"'`), so the output is "safe to directly output in HTML without additional escaping" — confirming D6.3's "echo directly, do NOT wrap in `esc_attr()`." `esc_html()` strips/encodes HTML-special characters for safe text-node output. What was **not executed in-worktree** (no running WP): the actual rendered escaping of a hostile `song` payload and the empty-state output — the Code phase should confirm via `wp-env` that a `song` containing `<script>`/`<`/`&`/quotes renders inert/escaped (AC8) and an empty block renders nothing (AC3).
...
**Sources:** [get_block_wrapper_attributes()](https://developer.wordpress.org/reference/functions/get_block_wrapper_attributes/) (returns a pre-`esc_attr()`-escaped attribute string, safe to output directly); [esc_html()](https://developer.wordpress.org/reference/functions/esc_html/) (encodes HTML-special characters for safe text output).
