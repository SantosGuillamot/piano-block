...
**No `source`; dynamic-block serialization.** As a dynamic block (`save: null`), the attribute has **no `source`**. The handbook states that *"attributes without a `source` will be automatically saved in the block comment delimiter"* [[1]](#refs) — so WordPress serializes `song` as JSON into the block's HTML-comment delimiter:

```html
<!-- wp:piano-block/piano {"song":"…escaped JSON string…"} -->
```

The block serializer handles JSON string-escaping of the value, so a string containing quotes, braces, `<`, or `&` round-trips faithfully (this is the documented mechanism; the actual save→reload round-trip in *this* plugin is AC1, exercised in the Code phase — see Section 12). On the front end the value is delivered to `render.php` as `$attributes['song']` (a PHP string) — the standard dynamic-block attribute path. (`source` is for parsing values out of saved *markup*; a dynamic block saves no markup.)

**Empty default → the "no song" sentinel.** The handbook also confirms the default semantics: a block attribute's `default` *"will be used if the `type` and `source` do not match anything within the block content"* [[1]](#refs). With `"default": ""`, a freshly inserted block has `song === ""`: the editor field renders blank and nothing is stored as song content (requirement 10, AC2). The **empty string is the canonical "no song yet" sentinel**, distinct from a present-but-minimal song (Section 3). Render keys off this (Section 8). Conformance validation applies only to **non-empty** input — the empty string is never "non-conformant."
...
- **Field:** `TextareaControl` from `@wordpress/components` — accessible, labeled, the simplest correct control for multi-line raw text (a monospace class can give it a code-like feel). Per the official Component Reference it accepts `label`, `help`, `value`, `onChange`, and `rows` [[2]](#refs). `PlainText` from `@wordpress/block-editor` is a chrome-less alternative recorded.
- **Error:** a `Notice` from `@wordpress/components` with `status="error"`, non-dismissible, rendered only when there are validation errors **and** the field is non-empty. The Component Reference documents `Notice`'s `status` prop (values include `warning | success | error | info`; `error` renders red) plus an `onDismiss` [[3]](#refs). The message is a clear, human-readable summary (the first error with its JSON path, or a short list). Using an **error-status** notice satisfies requirement 8's "clear, visible error" while remaining non-blocking — a `Notice` is presentational and does not prevent saving (the non-blocking behavior comes from the data flow above, not the notice severity).
...
| AC1 — `song` persists across save/reload | §6.1 (delimiter-comment serialization, doc-backed) — runtime round-trip confirmed in Code phase (§12.2) |
...
This worktree has **no `node_modules`** (`npm install` not run) and **no running WordPress**, so nothing in this design was *executed* here. The load-bearing WordPress claims fall into two distinct categories, which should not be conflated.

### 12.1 Doc-backed (verified against official WordPress documentation)

The following are **API contracts confirmed against `developer.wordpress.org`** (citations in Section 13). They are not "unverified guesses" — they are documented behavior of the WordPress Block API, core escaping functions, and `@wordpress/components`:

- **Attribute typing & serialization.** A `string` attribute may carry JSON as an intermediary; an attribute with no `source` is automatically saved in the block comment delimiter; a `default` is used when nothing matches in the block content. Source: Block Attributes handbook [[1]](#refs) (verbatim quotes in Section 6.1).
- **Escaping.** `esc_html()` encodes `&`, `<`, `>`, `"`, `'` for an HTML-block (text-node) context [[5]](#refs); `get_block_wrapper_attributes()` returns an already-escaped attribute string, so it is echoed directly (not re-escaped via `esc_attr()`) [[4]](#refs). Source: WordPress function references.
- **Editor components.** `TextareaControl` (from `@wordpress/components`) accepts `label` / `help` / `value` / `onChange` / `rows` [[2]](#refs); `Notice` (from `@wordpress/components`) takes a `status` prop whose values include `warning | success | error | info` (with an `onDismiss`) [[3]](#refs). Source: official Component Reference.

### 12.2 To confirm at runtime in the Code phase

What official docs **cannot** prove from here is the *end-to-end runtime behavior in this specific plugin* — which requires `npm run build` + a `wp-env` smoke test that this worktree cannot run. These are the genuine Code-phase verifications (each maps to an acceptance criterion):

- **AC1 — save→reload attribute round-trip.** That a `song` value set in the editor actually persists into the block-delimiter comment and reappears unchanged on reload of *this* block. (The serialization *mechanism* is doc-backed in §12.1; the realized round-trip in this plugin is what AC1 verifies.)
- **AC3 — empty renders nothing.** That a freshly inserted (or whitespace-only) block produces no element / no song content on the front end.
- **AC8 — XSS payload renders escaped.** That a `song` containing `<script>` / `<` / `&` / quotes actually renders escaped and inert on the front end. (The `esc_html()` behavior is doc-backed in §12.1; the realized escaping of a hostile payload in this plugin is what AC8 verifies.)
- **Installed `@wordpress/components` version.** The component **names and documented props** are confirmed (§12.1); the only open item is whether the *resolved/installed* package version's signatures match exactly (e.g. the precise non-dismissible prop name) — a quick check once `npm install` has run.

These verifications do not affect the data-model or format decisions (Sections 3–5, 7, 9), which are self-contained and independent of the WordPress runtime.

## 13. References

<a id="refs"></a>

1. WordPress Block Editor Handbook — Block Attributes: https://developer.wordpress.org/block-editor/reference-guides/block-api/block-attributes/ (string-type-as-JSON-intermediary; no-`source` → comment-delimiter serialization; `default`; allowed `type` values).
2. WordPress Component Reference — `TextareaControl`: https://developer.wordpress.org/block-editor/reference-guides/components/textarea-control/ (`label` / `help` / `value` / `onChange` / `rows`).
3. WordPress Component Reference — `Notice`: https://developer.wordpress.org/block-editor/components/notice (`status` ∈ `warning | success | error | info`; `onDismiss`).
4. WordPress Function Reference — `get_block_wrapper_attributes()`: https://developer.wordpress.org/reference/functions/get_block_wrapper_attributes/ (returns a pre-escaped attribute string).
5. WordPress Function Reference — `esc_html()`: https://developer.wordpress.org/reference/functions/esc_html/ (encodes text for an HTML block context).
